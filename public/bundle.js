...
},{"../transport":14,"component-inherit":11,"debug":21,"engine.io-parser":24,"parseqs":33,"xmlhttprequest-ssl":20,"yeast":53}],19:[function(require,module,exports){
(function (Buffer){
/**
 * Module dependencies.
 */
...
},{"../transport":14,"buffer":8,"component-inherit":11,"debug":21,"engine.io-parser":24,"parseqs":33,"ws":7,"yeast":53}],20:[function(require,module,exports){
...
},{"./on":39,"./socket":40,"backo2":3,"component-bind":9,"component-emitter":10,"debug":42,"engine.io-client":12,"indexof":31,"socket.io-parser":47}],39:[function(require,module,exports){
...
},{"./on":39,"component-bind":9,"component-emitter":10,"debug":42,"has-binary2":27,"parseqs":33,"socket.io-parser":47,"to-array":52}],41:[function(require,module,exports){
...
},{"dup":22,"ms":45}],44:[function(require,module,exports){
...
},{"./binary":46,"./is-buffer":48,"component-emitter":10,"debug":49,"isarray":44}],48:[function(require,module,exports){
(function (Buffer){
...
},{"./scripts/resumable":55,"./styles/main.scss":56,"jquery":32,"scssify":36,"socket.io-client":37}],55:[function(require,module,exports){
/*
 * MIT Licensed
...
(function () {
  ...
  var Resumable = function (opts) {
    ...
    function loadFiles(items, event) {
      ...
      var getSaved = (function getSavedFiles() {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            var myObj = JSON.parse(this.responseText);
            // console.log(myObj);
            // savedFiles.push(myObj);
            // return myObj;
            for (let obj of myObj) {
              savedFiles.push(obj.split('#')[2]);
            }
            // console.log(savedFiles);
          }
        };
        xmlhttp.open("GET", "/getfn", true);
        xmlhttp.send();
      });
      // getSavedFiles();
      if (!items.length) {
        ...
        function () {
          ...
          if (files.length) {
            for (let file of files) {
              ...
              // var identifier = files[i].uniqueIdentifier;
              //  console.log(files[i].name); 
              //  console.log(files[i].size); 
              // $h.generateUniqueIdentifier(files[i]).then(function (file) {});
              var id;

              function getId(file) {
                id = $h.generateUniqueIdentifier(file)
                  .then(function (result) {
                    // console.log(result);
                    // console.log(savedFiles);
                    if (savedFiles.includes(result)) {
                      // console.log(result);
                      // console.log(savedFiles);
                    } else {
            ...
            // appendFilesFromFileList(uploadFiles, event);
...
var css = "@import url(\"https://fonts.googleapis.com/css?family=Jura:400,700&display=swap&subset=cyrillic,cyrillic-ext\");@import url(\"../fonts/DS-Digital.css\");@import url(\"../styles/range.scss\");*{margin:0}html,body{height:100vh;width:100%}body{height:100%;display:flex;font:1em/1.5 'Jura', Sans-serif;margin:0;background:radial-gradient(circle, dimgray 0%, #000a0a 100%);flex-direction:column;align-items:center;justify-content:center;overscroll-behavior:contain}a{color:black;transition:.2s;cursor:pointer}a:hover{color:black;transition:.2s;text-shadow:0 0 0 #000}header,footer,main,div{box-sizing:border-box}header,footer{background:#465362;padding:1.5em;color:#ffffff}footer{text-align:center}header a,footer a,footer span{color:#ffffff;opacity:.5;transition:.3s}header a:hover,footer a:hover,header li.active a{opacity:1;transition:.3s}header nav ul{list-style:none;display:flex;flex-direction:row;flex-wrap:wrap}header nav ul li{margin:0 1em 0 0;line-height:1.2;padding:.5em}main{flex-grow:1}img,iframe{max-width:100%;height:auto;display:block}.flex-col,.flex-row{display:flex}.flex-row{flex-direction:row;flex-wrap:wrap}.flex-col{flex-direction:column}.flex-center{justify-content:center}.wrapper{width:100%;max-width:1200px;margin:1.5em auto;padding:0 .5em;text-align:center}.item{background-color:#ffffff;box-shadow:0 1px 1px 0 rgba(0,0,0,0.18);padding:1.5em;margin:.5em .5em}.wrapper .item>img{margin:0 auto}.wrapper .item span{display:inline-block;margin:1em 0 0 0}.wrapper .masonry{column-count:4;column-gap:1em;padding:0 .5em;overflow:auto}.wrapper .item{display:inline-block;margin:0 0 1em;width:100%}.upload-common{min-height:120px}.type-holder{margin:1.5em 1em 0 0}.type-holder select{padding:.6em 1em;font-size:1.25em}.upload-holder{overflow:hidden;position:relative;display:inline-block;background-color:#465362;color:#ffffff;padding:.75em 1em;margin:2em 0 1em;border-radius:.25em;font-size:1.25em;cursor:pointer}.upload-holder:hover{background-color:#3f667e}.upload-holder [type=file]{cursor:inherit;display:block;font-size:999px;filter:alpha(opacity=0);min-height:100%;min-width:100%;opacity:0;position:absolute;right:0;text-align:right;top:0}.progress-cancel-link{margin-left:10px}.resumable-error{font-size:14px;font-style:italic}.resumable-drop{padding:15px;font-size:13px;text-align:center;color:#666;font-weight:bold;background-color:#eee;border:2px dashed #aaa;border-radius:10px;z-index:9999;transition:all 0.5s}.resumable-dragover{padding:30px;color:#555;background-color:#ddd;border:1px solid #999;transition:all 0.5s}.resumable-progress{margin:30px 0 30px 0;width:100%}.progress-container{height:7px;background:#9CBD94;position:relative}.progress-text{font-size:11px;line-height:9px;padding-left:10px}.progress-pause{padding:0 0 0 7px}.is-paused .progress-resume-link{display:inline}.uploader-item{width:148px;height:90px;background-color:#666;position:relative;border:2px solid black;float:left;margin:0 6px 6px 0}.uploader-item-thumbnail{width:100%;height:100%;position:absolute;top:0;left:0}.uploader-item img.uploader-item-thumbnail{opacity:0}.uploader-item-creating-thumbnail{padding:0 5px;font-size:9px;color:white}.uploader-item-title{position:absolute;font-size:9px;line-height:11px;padding:3px 50px 3px 5px;bottom:0;left:0;right:0;color:white;background-color:rgba(0,0,0,0.6);min-height:27px}.uploader-item-status{position:absolute;bottom:3px;right:3px}.uploader-item:hover,.is-active .uploader-item{border-color:#4a873c;cursor:pointer}.uploader-item:hover .uploader-item-title,.is-active .uploader-item .uploader-item-title{background-color:rgba(74,135,60,0.8)}.is-error .uploader-item:hover,.is-active.is-error .uploader-item{border-color:#900}.is-error .uploader-item:hover .uploader-item-title,.is-active.is-error .uploader-item .uploader-item-title{background-color:rgba(153,0,0,0.6)}#frame{max-width:1000px;width:100%;height:100%}.videoWindow{width:100%;display:flex;flex-direction:row}.video-info{width:0%}#progress{width:100%;height:30px;border:1px solid black;border-radius:15px;overflow:hidden}#progress[value]{-webkit-appearance:none;appearance:none;width:250px;height:20px}#progress[value]::-webkit-progress-bar{background-color:rgba(238,238,238,0);border-radius:2px;box-shadow:0 2px 5px rgba(0,0,0,0.25) inset}#progress[value]::-webkit-progress-value{background-image:-webkit-linear-gradient(-45deg, transparent 33%, rgba(0,0,0,0.1) 33%, rgba(0,0,0,0.1) 66%, transparent 66%),-webkit-linear-gradient(top, rgba(255,255,255,0.25), rgba(0,0,0,0.25)),-webkit-linear-gradient(left, #333, #555);border-radius:2px;background-size:55px 30px, 100% 100%, 100% 100%;-webkit-animation:animate-stripes 5s linear infinite;animation:animate-stripes 5s linear infinite}@keyframes animate-stripes{100%{background-position:-100px 0px}}.fileList{height:50vh;overflow-y:auto;position:relative}.resumable-list{padding:0;overflow:auto;text-align:left}.resumable-list li{display:flex;flex-direction:row;justify-content:space-between;height:50px;align-items:center;overflow:hidden;font-weight:bold;background-color:#9c9c9c}.resumable-list li span{z-index:1}.resumable-file-name{width:65%;user-select:none}.resumable-list li a{text-decoration:none}.convert360p{background-color:#ffca7b}.convert720p{background-color:#ff977d}.convert1080p{background-color:#ff6c47}.resumable-list li button{font-family:'Jura';font-weight:bold;text-decoration:none;text-transform:capitalize;border-radius:7px;border:1px solid black;white-space:nowrap;font-size:15px;padding:10px;margin:0 10px;cursor:crosshair}.resumable-list li button:disabled{color:#ffffff;cursor:default}.backgnd-status{position:absolute;background-color:#c8c8c8;width:100%;height:49px}.resumable-file-progress{display:flex;flex-direction:row}.resumable-file-size{padding:0 5px;height:30px;z-index:5;background-color:rgba(0,0,0,0.18);transition:transform 500ms}.footer{height:150px}button{border:none;background:none;text-decoration:none}.media{display:flex;flex-direction:column}.media>video{width:100%;max-height:563px}.media>.navigationBar{display:flex;flex-direction:row;justify-content:space-between}.media>.navigationBar>.navigationButtons{display:flex;flex-direction:row}.media>.navigationBar>.navigationButtons>button{width:100px;border:2px solid black;border-radius:10px;margin:10px 5px;font-size:20px;font-weight:bold;color:#000}.media>.navigationBar>.navigationButtons>button.play{background-color:green;color:#fff}.media>.navigationBar>.navigationButtons>button.playActive{background-color:brown;color:#fff}.media>.navigationBar>.navigationTimer{display:flex;flex-direction:column;font-family:'DS-Digital';font-weight:bold;font-size:70px;line-height:70%}\n"
